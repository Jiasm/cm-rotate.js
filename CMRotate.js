...
var CMRotate = CMRotate || (function () {
  var _public = {}
  var $contaier
  var _isTouch = false // 'ontouchstart' in window
  var _isTrans3D
  var _cssTransform
  var PI = Math.PI / 180
  var _radius
  ...
  function render (pos, sita, no) {
    ...
    if (sita > 10 && sita < 170) {
      ...
      if (pos.item == null) {
        ...
        prev = _posArr[getInsideMax(no + prevV, _posTotal)]
        id = getInsideMax(prev.id + idV, _bgTotal)
  ...
  function circlePos (sita) {
    ...
    var sin = Math.sin(sita * PI)
    var imgX = (cos) * _radius + _centerX - _itemHW
    var imgY = (sin) * _radius + _centerY - _itemHH
